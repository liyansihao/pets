...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  // Map page path to page type
  const getPageFromPath = (): Page => {
    if (pathname === '/') return 'home';
    if (pathname.startsWith('/pricing')) return 'pricing';
    if (pathname.startsWith('/my-creations')) return 'my-creations';
    return 'home';
  };

  const currentPage: Page = getPageFromPath();
  ...
  const handleNavigate = (page: Page) => {
    ...
    else if (page === 'my-creations') router.push('/my-creations');
    else if (page === 'billing') router.push('/billing');
  };
              ...
              {mappedUser ? (
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-100 dark:border-slate-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all py-2 z-50">
                    <button
                      onClick={() => handleNavigate('my-creations')}
